...
//SSL
	...
	loginRequest.setUserEmail("Test@postman.com");
	loginRequest.setUserPassword("Test@post1");
	
		
	RequestSpecification reqLogin =given().relaxedHTTPSValidation().log().all().spec(req).body(loginRequest);
	String Res = reqLogin.when().post("/api/ecom/auth/login").then().log().all().extract().response()
			.asString();
	...
	.multiPart("productImage",new File("C:\\Users\\BB2712\\Pictures\\Screenshots\\Laptops.png"));
	...
	RequestSpecification createOrderBaseReq=	new RequestSpecBuilder().setBaseUri("https://rahulshettyacademy.com")
			...
			.build();
	OrderProductDetails orderDetail = new OrderProductDetails();
	orderDetail.setCountry("India");
	orderDetail.setProductOrderedId(productId);
	
	List<OrderProductDetails> orderDetailList = new ArrayList<OrderProductDetails> ();
	orderDetailList.add(orderDetail);	
	PlaceOrder orders = new PlaceOrder();
